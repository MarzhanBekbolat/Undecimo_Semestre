...
/*****************************************************************************
  ...
  simple_bus.cpp : The bus.
		   ...
		   round (request wih higher priority preceeded), then the
...
{
nivel_utilizacion nivel_util;  ///creamos un objeto de la clase nivel de utilizacion para 
double time;
double ocup1=0; 				//calcular el porcentaje de nivel de utilizacion del bus cada vez que este es requerido
  ...
  if (!m_current_request){    //indica q ningun maestro requiere el bus
...
if (m_current_request){   /// condicion de peticion del bus
		handle_request();
//nivel de utilizacion


//sb_fprintf(stdout, "%g\tNivel de utilización666:\t%g%\n", sc_time_stamp().to_double(), des);
contador_n++;
...
//bus desopcupado
des++;
//sb_fprintf(stdout, "%g\tdesocupado:\t%g%\n", sc_time_stamp().to_double(), des);

  	//nivel_util.desocupado();
}
  if (!m_current_request){   //se culmina el uso del bus
    clear_locks();
  }
//actual_time=sc_time_stamp().to_double();
//se invoca el calculo para el nivel de utilización del bus
  nivel_util.utilizacion(ocup1);
