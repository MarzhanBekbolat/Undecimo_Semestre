...
simple_bus_arbiter::arbitrate(const simple_bus_request_vec &requests)
{
	  ...
	  simple_bus_request *request = requests[i];
          ...
          // simple_bus_status encoding
           sb_fprintf(stdout, "\n    R[%d](%c%s@%x)",
                     ...
                     simple_bus_status_str[request->get_status()],
                     request->address);
	}
    }
  ...
  for (i = 0; i < requests.size(); ++i)
    {
      simple_bus_request *request = requests[i];
      if ((request->get_status() == SIMPLE_BUS_WAIT) &&
  ...
  for (i = 0; i < requests.size(); ++i)
    {
      ...
      if (requests[i]->order ==1)
			best_request = requests[i];
    }
  /*static unsigned int turn=4;
  if(turn>3)turn=1;else turn++;
  for (i = 0; i < requests.size(); ++i)
    {
      if (requests[i]->priority ==turn)
			best_request = requests[i];
    }*/
